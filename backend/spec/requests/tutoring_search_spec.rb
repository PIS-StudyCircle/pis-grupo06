...
RSpec.describe "Tutorings search", type: :request do
  ...
  let!(:user)       { User.create!(email: "test@test.com", password: "12345678", password_confirmation: "12345678", name: "Test", last_name: "User", faculty: faculty) }
  ...
  let!(:tutoring1) { Tutoring.create!(course: calculo, scheduled_at: 1.day.from_now, duration_mins: 60, modality: "virtual", capacity: 5, subjects: [derivadas, integrales]) }
  let!(:tutoring2) { Tutoring.create!(course: calculo, scheduled_at: 2.days.from_now, duration_mins: 60, modality: "presencial", capacity: 5, subjects: [derivadas]) }
  let!(:tutoring3) { Tutoring.create!(course: fisica, scheduled_at: 3.days.from_now, duration_mins: 60, modality: "virtual", capacity: 5, subjects: [vectores]) }
  ...
  describe "GET /api/v1/tutorings?search=..." do
    it "filtra por materia (course) con coincidencia parcial y sin acentos" do
      ...
      ids = JSON.parse(response.body)["tutorings"].map { |t| t["id"] }
    ...
    it "devuelve vacío si no hay coincidencias por materias" do
      ...
      expect(response).to have_http_status(:ok)
      tutorings = JSON.parse(response.body)["tutorings"]
    ...
    it "filtra correctamente cambiando de materia a tema y viceversa" do
      ...
      expect(JSON.parse(response.body)["tutorings"].map { |t| t["id"] }).to eq([tutoring3.id])

      get "/api/v1/tutorings", params: { search: "int", search_by: "subject" }
      expect(JSON.parse(response.body)["tutorings"].map { |t| t["id"] }).to eq([tutoring1.id])

      get "/api/v1/tutorings", params: { search: "calc", search_by: "course" }
      expect(JSON.parse(response.body)["tutorings"].map { |t| t["id"] }).to eq([tutoring1.id, tutoring2.id])
    end
  end
end