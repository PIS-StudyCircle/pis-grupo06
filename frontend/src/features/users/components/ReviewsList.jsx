...
export default function ReviewsList({ reviews, onUpdate }) {
    ...
    <ul className="space-y-3">
        ...
        return (
            <li key={rev.id} className="border rounded-lg p-3 bg-gray-50 text-sm relative">
            {/* --- Si está en edición --- */}
            {editingId === rev.id ? (
            <form
                onSubmit={(e) => {
            ...
            <div className="absolute top-2 right-2 flex gap-2">
                <button
                    ...
                    >
                    <Pencil size={18} strokeWidth={1.75} color="black" />
                </button>
                <button
                    ...
                    className="text-black hover:opacity-70 transition-opacity"
                >
                    <Trash2 size={18} strokeWidth={1.75} color="black" />
                </button>
            </div>
            )}
            </li>
